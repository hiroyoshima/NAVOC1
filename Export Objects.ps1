function Export-Nav-Objects {
    ...
    $ParentPath = "$Location\$VersionList"
    ...
    $ObjectFilter = "Version List=*$VersionList*"
    $LogFileDev = "`"$ParentPath\QueryExportLogTxt.txt`""
    $FileDevTxt = "$ParentPath\1 DEV_$VersionList.txt"
    $FileDevFob = "$ParentPath\2 DEV_$VersionList.Fob"
    $FileLiveTxt = "$ParentPath\1 LIVE_$VersionList.txt"
    $FileLiveFob = "$ParentPath\2 LIVE_$VersionList.Fob"
    
    #Check if RunAsDate file
    $RunAsDateExeFile = "D:\Program Files\RunAsDate\RunAsDate.exe"
    if (!(Test-Path $RunAsDateExeFile)) { Throw "$RunAsDateExeFile RunAsDate.exe file cannot be found" }
    
    # Check finsql.exe file
    $FinSQLExeFile = "C:\Program Files (x86)\Microsoft Dynamics NAV\90\RoleTailored Client\finsql.exe"
    if (!(Test-Path $FinSQLExeFile)) { Throw "$FinSQLExeFile finsql.exe file cannot be found" }

    #$DeltaFile = "$ParentPath\0 DELTAFILE_$VersionList.txt"
    $ExportSript = "`"$RunAsDateExeFile`" /movetime 26\06\2018 00:00:00 "
    $ExportSript += "`"$FinSQLExeFile`" command=exportobjects, "
    $ExportSript += "servername=$ServerName, username=$DatabaseUsername, password=$DatabasePassword, filter=$ObjectFilter,"
    ...
    & cmd /c "$ExportSript database=$DatabaseNameDev, file=$FileDevTxt, ntauthentication=no, logfile=$LogFileDev"
    # Allow Export DevFob
    if ($ExportDevFob) { & cmd /c "$ExportSript database=$DatabaseNameDev, file=$FileDevFob, ntauthentication=no, logfile=$LogFileDev" }
    
    & cmd /c "$ExportSript database=$DatabaseNameLive, file=$FileLiveTxt, ntauthentication=no, logfile=$LogFileDev"
    # Allow Export LiveFob
    if ($ExportLiveFob) { & cmd /c "$ExportSript database=$DatabaseNameLive, file=$FileLiveFob, ntauthentication=no, logfile=$LogFileDev" }
